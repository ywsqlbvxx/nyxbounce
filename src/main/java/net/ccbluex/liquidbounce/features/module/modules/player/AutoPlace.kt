...
object AutoPlace : Module("AutoPlace", Category.PLAYER, gameDetecting = false) {
    ...
    val onUpdate = handler<UpdateEvent> {
        ...
        if (fastPlaceOnJump.get() && holdRight.get() && !ModuleManager.fastPlace.state && Mouse.isButtonDown(1)) {
            if (thePlayer.motionY > 0.0) {
                mc.rightClickDelayTimer = 1
            } else if (!pitchCheck.get() || thePlayer.rotationPitch >= 70f) {
        ...
        if (disableLeft.get() && Mouse.isButtonDown(0))
            return@handler
        ...
        if (lastRayTrace != null && frameCount < frameDelay.get()) {
            frameCount++
            return@handler
        }
        ...
        if (holdRight.get() && !Mouse.isButtonDown(1))
            return@handler

        val currentTime = System.currentTimeMillis()
        if (currentTime - lastPlace < minPlaceDelay.get())
            return@handler
